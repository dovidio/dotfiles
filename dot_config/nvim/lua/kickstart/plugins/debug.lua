...
return {
  ...
  config = function()
    ...
    dap.adapters.php = {
      type = 'executable',
      command = 'node',
      args = { '/Users/umbertodovidio/Development/vscode-php-debug/out/phpDebug.js' },
    }
    ...
    dap.configurations.go = {
      -- Debug the current test function
      {
        ...
        args = function()
          local test_name = vim.fn.expand '<cword>' -- Gets word under cursor
          if test_name:match '^Test' then
            return { '-test.run', '^' .. test_name .. '$' }
          else
            return { '-test.run', vim.fn.input 'Test function name: ' }
          end
        end,
      },
